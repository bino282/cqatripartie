...
class Model:
	def __init__(self,training=True,type=3):
		...
		if(type==2):
			self.net=self.build_network(self.sentence1,self.sentence2,self.num_classes)
		if(training):
			...
			self.optimizer = tf.train.GradientDescentOptimizer(self.learning_rate).minimize(self.cost, global_step = self.batch)
	...
	def build_network(self,sentence1,sentence2,sentence3,num_outputs,keep_prob = settings.dropout,training = True):
		...
		return net
	def build_netword_2line(sentence1,sentence2,num_outputs,keep_prob = settings.dropout,training = True):
