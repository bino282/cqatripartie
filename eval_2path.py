...
model=model.Model(training=False,type=2)
...
for f in list_file_data:
    ...
    for batch in data:
        ind=0
        acc_avg_batch=0
        confusion_matrix_tf=sess.run([model.confusion_matrix],feed_dict={model.sentence1:batch[0],model.sentence2:batch[1],model.y_true:batch[3]})
